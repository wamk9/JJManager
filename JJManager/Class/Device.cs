...
namespace JJManager.Class
{
    public class Device : INotifyPropertyChanged
    {
        ...
        private string FindSerialPortByDevice()
        {
            // Get the COM port names (e.g., COM1, COM2, etc.)
            ...
            using (var results = searcher.Get())
            {
                foreach (var mo in results.Cast<ManagementObject>())
                {
                    ...
                    string deviceID = mo["DeviceID"]?.ToString(); // e.g., "COM3"
                    string pnpDeviceID = mo["PNPDeviceID"]?.ToString(); // e.g., "USB\\VID_1234&PID_5678\\SERIALNUMBER"
                    string name = mo["Name"]?.ToString(); // e.g., "USB Serial Port (COM3)"

                    Console.WriteLine($"DeviceID: {deviceID}");
                    Console.WriteLine($"PNPDeviceID: {pnpDeviceID}");
                    Console.WriteLine($"Name: {name}");

                    // Compare the PNPDeviceID with the one from HIDSharp
                    // Here, match with the relevant HID device identifiers
                    // Assuming you already know the VID and PID from HIDSharp

                    // Example check (replace with your actual VID & PID):
                    if (pnpDeviceID.Contains("VID_1234") && pnpDeviceID.Contains("PID_5678"))
                    {
                        Console.WriteLine($"Found matching COM port: {deviceID}");
                        // You can break or return here if you found your desired COM port
                        break;
                    }
            ...
            foreach (ManagementObject queryObj in searcher.Get())
            {
                ...
                string name = queryObj["Caption"].ToString(); // e.g., "USB Serial Port (COM3)"
                string deviceID = queryObj["DeviceID"].ToString(); // e.g., "USB\\VID_1234&PID_5678\\SERIALNUMBER"

                // Check if the serial number from the device matches the serial port's device ID
                if (deviceID.Contains(HidDevice.DevicePath))
                {
                    // Extract the COM port name (e.g., "COM3")
                    string comPort = name.Substring(name.LastIndexOf("(COM")).Replace("(", "").Replace(")", "");
                    return comPort;
                }
            }*/

            return null; // No matching serial port found
        }
